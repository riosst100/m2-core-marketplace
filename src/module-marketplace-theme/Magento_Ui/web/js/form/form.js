...
define([
    ...
    return Collection.extend({
         ...
         */
        reload: function () {
            var dataParamsArr = $(':input').serializeArray(), 
                dataParams = {};
            
            $(dataParamsArr).each(function(i, field){
                var str = field.name;
                var name = str.split('[').pop().split(']')[0];
                dataParams[name] = field.value;
            });
            ...
            dataParams['category_id'] = category_id;
            
            var dataParams = $.extend({}, this.params, dataParams);
            makeRequest(dataParams, this.data, this.reloadUrl).then(function (data) {
                app(data, true);
            });
        }
    });

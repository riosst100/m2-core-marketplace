 ...
 */
class AutoBids extends Extended
{
    /**
     * Core registry
     *
    ...
    {
        parent::_construct();
        $this->setId('autobids_grid');
        $this->setDefaultSort('entity_id');
        $this->setUseAjax(true);
        if ($this->getProduct() && $this->getProduct()->getProductId()) {
            $this->setDefaultFilter(['in_autobids' => 1]);
        }
        if ($this->isReadonly()) {
    ...
    {
        return $this->_coreRegistry->registry('current_autobids');
    }
    ...
    {
        $collection = $this->_autoauction->getCollection()
            ->addFieldToFilter('auction_id', $this->getProduct()->getId());

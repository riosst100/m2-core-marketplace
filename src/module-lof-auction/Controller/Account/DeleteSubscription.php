 ...
 */
class DeleteSubscription extends AbstractAccount
{
    /**
     * @var Session
     */
    ...
    public function execute()
    {
        ...
        if ($customerId && $id && $this->_helperData->isEnabled()) {
            try {
                $foundRecord = $this->_maxAbsenteeBidRepository->getById($id);
                if ($foundRecord && $foundRecord->getCustomerId() == $customerId) {
                    $this->_maxAbsenteeBidRepository->delete($id);
                    $this->messageManager->addSuccess(__('Your max abstentee bid amount successfully deleted.'));
                } else {
                    $this->messageManager->addError(__("You dont have permission!"));
                }

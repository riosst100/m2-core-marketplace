...
/**
 * Class SetMaxabsentee
 * Lof\Auction\Controller\Account
 */
class SetMaxabsentee extends AbstractAccount
{
    /**
     * @var Session
     */
    ...
    public function execute()
    {
        ...
        $maxBidAmount = isset($data["max_absentee_bid"]) ? (float)$data["max_absentee_bid"] : 0;
        $customerId = $this->_customerSession->getCustomerId();
        if ($auctionId && $customerId && $maxBidAmount && $this->_helperData->isEnabled() && $this->_helperData->getConfig("auto/enable_max_absentee_bid")) {
            try {
                $this->_maxAbsenteeBidRepository->setMaxBidAmount($customerId, $auctionId, $maxBidAmount);
                $this->messageManager->addSuccess(__('Your max abstentee bid amount successfully saved.'));
            } catch (\Exception $e) {

 ...
 */
class MassIsSpam extends Autobid
{
    /**
     * @var Filter
     */
    ...
    public function execute()
    {
        ...
        foreach ($collection as $bid) {
            ...
            $this->updateMixAmount($bid, 1);
        }

        $this->messageManager->addSuccess(__('A total of %1 record(s) have been spam.', $collectionSize));
     ...
     * @return void
     */
    protected function updateMixAmount($bid, $status = 1)
    {
        try {
    ...
    {
        return $this->_authorization->isAllowed('Lof_Auction::autobid_save');
    }
}

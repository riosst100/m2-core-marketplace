 ...
 */
class MaxAbseteeBid implements ResolverInterface
{
    /**
     * @var GetCustomer
     */
    ...
    public function resolve(
        ...
        if (!isset($args['product_id']) || !$args['product_id']) {
            throw new GraphQlInputException(__('Missing required param product_id.'));
        }
        if (!isset($args['amount']) || !$args['amount']) {
            throw new GraphQlInputException(__('Missing required param amount.'));
        }
        $customer = $this->getCustomer->execute($context);
        return $this->absenteeRepository->setMaxBidAmount($customer->getId(), $args['product_id'], $args['amount']);
    }
}

...
namespace Lofmp\Auction\Controller\Marketplace\AutoBid;
...
class Delete extends \Lofmp\Auction\Controller\Marketplace\Delete
{
    ...
    public function __construct(
        ...
        $this->autoFactory = $autoAuctionFactory;
    ...
    public function execute()
    {
        ...
        try {
            ...
            if (!empty($fields)) {
                $model = $this->autoFactory->create()
                    ->load($fields['id']);
                if (!empty($model)) {
                    if ($this->validate($model->getAuctionId())) {
                        $model->delete();
                        $this->messageManager->addSuccess(__('Manual bid is successfully Deleted!'));
                    }

...
/**
 * Class GuestSellerDeliverySlot
 * @package Lofmp\DeliverySlot\Model
 */
class GuestSellerDeliverySlot implements GuestSellerDeliverySlotInterface
{
    /**
        ...
        if ($this->data->getDeliverySlotConfig('enabled') != 0) {
            ...
            foreach ($seller_ids as $sellerId) {
                ...
                if ($this->data->checkSellerVacationMode($target_date, $sellerId ) == 1) {
                    ...
                    $sellerNameText = $sellerName?(" by Seller ".$sellerName):"";
    ...
    protected function getQuote($cartId)
    {
        if (null === $this->quote) {
            ...
            /** @var Quote $quote */
            $this->quote = $this->cartRepository->getActive($quoteIdMask->getQuoteId());
        }
        return $this->quote;
    }
